...
% cF_max = 500; %vp.MaxRollFriction_N;
...
w_stm = 1; %0.5*(tanh((vx_mps - vx_b)/vx_d) + 1);
% w_km = 1 - w_stm; 
% blend process
dvx = dvx_stm * w_stm; %dvx_km * w_km +;
dvy = dvy_stm * w_stm; % dvy_km * w_km + 
ddPsi = ddPsi_stm * w_stm; %ddPsi_km * w_km + 
domega_rad = domega_stm * w_stm; %domega_km * w_km + 
dalpha_rad = dalpha_rad_stm * w_stm; %dalpha_rad_km * w_km + 
dlambda_perc = dlambda_perc_stm * w_stm; %dlambda_perc_km * w_km +  
ax = ax_stm * w_stm; %ax_km * w_km + 
ay = ay_stm * w_stm; %ay_km * w_km + 
