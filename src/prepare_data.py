...
def create_dataset_separation_run(data_test: np.array,
    ...
    vx = data_test[start + input_timesteps:start + duration, 0]
    vy = data_test[start + input_timesteps:start + duration, 1]
    yaw_rate = data_test[start + input_timesteps:start + duration, 2]
    ...
    return initials,vx, vy, yaw_rate, acc_x, acc_y, steeringangle_rad, torqueRL_Nm, torqueRR_Nm, brakepresF_bar, brakepresR_bar,wf,wr
...
def create_dataset_separation(path_dict: dict,
    ...
    axay_true = np.zeros((lengthsum, 2))
    ...
    for u in range(0, file_counting):
        ...
        axay_true[lengthsumslips:lengthsumslips + len(data[u]) - input_timesteps] \
            = (data[u])[input_timesteps-1:-1, 3:5]
    ...
    axay_train = axay_true[0:(p // input_timesteps), :]
    ...
    axay_val = axay_true[(p // input_timesteps):, :]
